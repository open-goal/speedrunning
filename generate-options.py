...
def generate_option_string(listVarName, label, offset, type, size, name):
  return "AddOption(vars.{}, \"{}\", {}, typeof({}), {}, false, \"{}\", false);".format(listVarName, label, offset, type, size, name)
...
def generate_options(input_file, output_file):
  ...
  with open(input_file, 'r') as f:
    ...
    for option in options["structFields"]:
      if "hidden" in option and option["hidden"] is not True:
        # Add to manual settings
        option_name = option["label"]
        if "name" in option:
          option_name = option["name"]
        manual_settings.append(generate_option_string("manual_options", option["label"], curr_offset, type_to_typename(option["type"]), type_to_offset(option["type"]), option_name))
